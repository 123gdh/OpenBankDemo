...
public class ScheduledTask {
    ...
    public void overdueVerification(){
        try {
            ...
            log.error("定时任务：定时校验十天未处理完成的申请单异常: {}",e.getMessage());
        }
    }
     ...
     */
    @Scheduled(cron = "0 0/2 * * * ?")
    public void updateOpenBankStatus(){
        try {
     ...
     */
    @Scheduled(cron = "0 0/2 * * * ?")
    public void updateOpenBankTransferStatus(){
        try {
   ...
   */
    @Scheduled(cron = "0 0/2 * * * ?")
    public void updateOpenBankBatchTransferStatus(){
        try {
            ...
            for (OpenBankBatchTransferTimingDao openBankBatchTransferTimingDao : openBankBatchTransferTimingDaos) {
                    ...
                    new QueryDetailsUtils().queryBatchTransfers(openBankBatchTransferTimingDao.getOut_batch_transfer_id(),requestOptions);
            }
        } catch (EntpayException e) {
